...
## Install via source
This is not recommended long-term, but it is sufficient for playing around with FauxRPC.
```shell
go install github.com/sudorandom/fauxrpc/cmd/fauxrpc@latest
```

## Run with `go run`
This is also not recommended long-term, but it is sufficient for playing around with FauxRPC.
```shell
